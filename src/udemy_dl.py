...
logger = colorlog.getLogger('udemy_dl')
...
def logging_exception(type_, value, tb):
    ...
    logger.exception(''.join(traceback.format_exception(type_, value, tb)))
...
def extract_lecture_url(course_id, lecture_id, asset_type):
    ...
    get_url = 'https://www.udemy.com/api-2.0/users/me/subscribed-courses/{0}/lectures/{1}?fields[asset]=@min,download_urls,external_url,slide_urls&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4'.format(
    ...
    if ('Video' or 'VideoMashup') in asset_type:
        return parse_video(json_source)
    elif 'E-Book' in asset_type:
        return parse_ebook(json_source)
    elif 'Presentation' in asset_type:
        return parse_presentation(json_source)
    elif 'Article' in asset_type:
        return [get_url, 'Article']
    elif 'Audio' in asset_type:
        return parse_audio(json_source)
    elif 'File' in asset_type:
        return parse_file(json_source)
    else:
...
def parse_video(json_source):
    ...
    list_m3u8 = re.findall(r'source\s+src="(.*?)"\s+.*?\s+data-res="\w+"', json_source['view_html'], re.DOTALL)
    list_videos = re.findall(r'source\s+src="(.*?)"\s+.*?\s+data-res="(\d+)"', json_source['view_html'], re.DOTALL)
...
def parse_presentation(json_source):
    ...
    for element in json_source['asset']['download_urls']['Presentation']:
        if element['label'] == 'download':
...
def parse_quiz(quiz_id):
    ...
    quiz_url = 'https://www.udemy.com/api-2.0/quizzes/{0}/assessments?fields[assessment]=@all&page_size=250'.format(quiz_id)
...
def get_attached_file(attached_file_dir, attached_info):
    ...
    for item in attached_list:
        attached_url = 'https://www.udemy.com/api-2.0/users/me/subscribed-courses/{0}/lectures/{1}/supplementary-assets/{2}?fields[asset]=download_urls'.format(
            course_id, lecture_id, item['id'])
...
def save_link(link, dest, data_type):
    ...
    if 'Article' in data_type:
        ...
        logger.warning('Video contains m3u8 file. it is better to download instead of saving links')
...
def check_course_status(course_id):
    ...
    try:
        check_course_status_url = 'https://www.udemy.com/api-2.0/users/me/course-previews/?course={0}'.format(course_id)
...
def prompt_for_password(prompt_text):
    # Workaround for https://github.com/nishad/udemy-dl/issues/92
...
def main():
    ...
    if debug_status:
        ...
        logging.basicConfig(filename=logging_name, filemode='w', level=0, format='%(asctime)s - [%(levelname)s-%(name)s-%(lineno)d] - %(message)s')
